...
//	ifstream freq_input; freq_input.open("frequency.txt");
	...
	for(int i = 0; i < 1000; ++ i)
	{
		...
		for(int j = 0; j < 5; ++ j)
		{
			double inc = mrand() % 10000001 / 10000000.0;
			for(int k = 0; k < 26; ++ k)
			{
				inc -= prob[k];
				if(inc < 0)
	...
	ofstream entropy_output; entropy_output.open("diffbig_small_hardmode.txt");
/*
	for(auto v : ori_word)
...
*/
	...
	for(auto u : ori_word)
	{
		...
		cerr << diff_res.size() * 100.0 / ori_word.size() << "% " << diff << endl;
	}
